# -*- coding: utf-8 -*-
from qrmaker.constant import alig_location, format_info_str, version_info_str, lindex
...
def add_an_alignment(row, column, m):
    for i in range(row-2, row+3):
        for j in range(column-2, column+3):
            m[i][j] = 1 if i in (
                row-2, row+2) or j in (column-2, column+2) else 0
    m[row][column] = 1
    ...
    for i in range(8):
        ...
        for row in range(len(mp)):
            for column in range(len(mp)):
                mp[row][column] = 1 if mp[row][column] is None and formula(
                    i, row, column) else 0
        mps.append(mp)
